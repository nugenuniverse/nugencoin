...
// setClaimPeriod
    ...
    constructor(address tokenAdd,uint subsequentClaims, uint releaseClaims, uint vestingPeriod, uint cliffs, uint totalNumberOfVestings) {
        require(vestingPeriod > 0, "Reserve  : timestamp > 0");
        require(subsequentClaims> 0, "Reserve  : timestamp > 0");
...
/// add distribution mention address, amount and local timestamp
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 250,"Reserve : length < 250");
        require((account.length == amount.length) && (startTime.length == amount.length),"Reserve : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
                require ( account[i] != 0x0000000000000000000000000000000000000000, "remove the zero address");
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Reserve : insufficient balance to allocate");
...
/// revoke distribution from beneficiary address , amount credit in admin wallet
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        ...
        for(uint i=0; i< account.length; i++) {  
            ...
            require(user_.totalClaimed < user_.balance, "Reserve : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "Reserve : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"Reserve : revoke transfer failed");
            }
                  emit revokewallet(
                account[i]
            );
...
///only beneficiary wallet will claim after vesting start
    function claim() external {
        ...
        require(user_.balance > 0, "Reserve : user not exist");
        require(user_.totalClaimed < user_.balance, "Reserve : total claim < total balance");
        require((user_.lastClaim > 0) && (user_.lastClaim <= block.timestamp), "ReserveL : lastClaim < block.timestamp");
        ...
        require(claimAmount > 0, "Reserve : claimAmount is zero");
...
/// extra token withdraw by admin
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Reserve : insufficient balance");
            require(payable(owner()).send(amount), "Reserve : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Reserve : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Reserve : transfer failed");
        }
    }
}
